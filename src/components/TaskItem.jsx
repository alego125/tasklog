    ...
    <div id={`task-${task.id}`} style={{ borderTop:'1px solid #1e293b', background:isExp?cfg.bg:'transparent', transition:'background .2s' }}>
      <div className="ft-task-row" style={{ padding:'11px 16px', display:'flex', alignItems:'center', gap:10, borderLeft:`3px solid ${cfg.border}` }}>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          <div onClick={() => onExpand(isExp ? null : task.id)} style={{ fontWeight:600, fontSize:14, textDecoration:task.done?'line-through':'none', color:task.done?'var(--text-faint)':(status==='overdue'?'#ef4444':status==='warning'?'#f59e0b':'var(--task-title)'), wordBreak:'break-word', overflowWrap:'break-word', cursor:'pointer' }} title={isExp?'Colapsar':'Expandir bitácora'}>
            {task.title}
          </div>
