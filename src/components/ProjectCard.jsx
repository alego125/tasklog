...
export default function ProjectCard({
  ...
  return (
    <div id={`project-${project.id}`} className="neu-card" style={{ background:'var(--bg-surface)', border:`1px solid ${hasOverdue?'#cba8a844':'var(--border-soft)'}`, borderRadius:14, marginBottom:16, overflow:'hidden' }}>
        ...
        <div>
          ...
          {mixedItems.map((item, idx) => {
            if (item._type === 'note') return (
              <div key={`note-${item.id}`} style={{ borderTop:idx===0?'none':'1px solid var(--border)', borderLeft:'3px solid var(--accent)' }}>
                <div style={{ padding:'10px 16px', display:'flex', alignItems:'flex-start', gap:10, background:'var(--bg-elevated)' }}>
                  <div style={{ flexShrink:0, marginTop:3 }}>
                    <span style={{ background:'var(--bg-hover)', border:'1px solid var(--accent)', color:'var(--accent)', padding:'2px 7px', borderRadius:10, fontSize:10, fontWeight:700 }}>📝 NOTA</span>
                  </div>
                  ...
                  <div style={{ display:'flex', gap:4, flexShrink:0 }}>
                    <button onClick={() => onMoveNote(item, project.id)} title="Mover a tarea" style={{ ...S.iconBtn, borderColor:'var(--accent)44', color:'var(--accent)' }}>🔀</button>
      ...
      {/* Resumen colapsado */}
      {isCollapsed && (
        <div style={{ padding:'10px 16px', borderTop:'1px solid var(--border)', display:'flex', gap:16, fontSize:12, color:'#FFFBEC' }}>
